...
window.addEventListener("scroll", () => {
  ...
  saluteTextContainer.style.transform = `translateY(${offsetY * 0.15}px)`;
  saluteImgContainer.style.transform = `translate(${offsetY * 0.4}px, ${
    offsetY * 0.7
  }px)`;
  authorName.style.transform = `translateX(${offsetY * 0.6}px)`;
  jobTitleContainers[0].style.backgroundPositionY = `${offsetY * 0.7}px`;
  jobTitleContainers[1].style.backgroundPositionY = `${-offsetY * 0.7}px`;
  jobTitles[0].style.transform = `translateX(calc(150vh - ${offsetY}px))`;
  jobTitles[1].style.transform = `translateX(calc(-170vh + ${offsetY}px))`;
  projectsTitle.style.transform = `translateY(calc(250vh - ${offsetY}px))`;
  phones[0].style.transform = `translateX(calc(280vh - ${offsetY}px))`;
  laptop.style.transform = `translateX(calc(400vh - ${offsetY}px))`;
  phones[1].style.transform = `translateX(calc(500vh - ${offsetY}px))`;
});
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  function typeWriter(text, i, fnCallback) {
    ...
    if (i < text.length) {
      ...
      document.querySelector(".saluteTextsmallTitle").innerHTML =
